...
async function istek(yol, secenekler = {}) {
  const [kaynak, sorgu] = yol.split("?");
  const adres = `/api/db?kaynak=${encodeURIComponent(kaynak)}${sorgu ? `&${sorgu}` : ""}`;
...
/* ---------- Sandık ---------- */
export function sandigimiGetir() {
  return istek("envanter?select=*&order=olusturma.desc");
}

/* Yönetici: herkesin sandığı (RLS admin_mi() ile korunuyor) */
export function tumEnvanter() {
  return istek("envanter?select=*&order=olusturma.desc&limit=500");
}

export async function esyaEtkinlestir(esyaId, nick, sunucu) {
  const res = await fetch("/api/envanter/etkinlestir", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "same-origin",
    body: JSON.stringify({ esyaId, nick, sunucu }),
  });
...
/* ---------- Etkinlik çarpanı ---------- */
export async function carpaniGetir() {
  try {
    const res = await fetch("/api/etkinlik", { cache: "no-store" });
...
export function carpaniAyarla({ aktif, carpan, bitis, baslik }) {
  return rpc("carpan_ayarla", {
    p_aktif: aktif,
    p_carpan: carpan,
    p_bitis: bitis,
    p_baslik: baslik,
